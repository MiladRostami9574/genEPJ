#!/usr/bin/env python3
"""Core model-generation utilities for genEPJ.

This module orchestrates template-based changes, modifications, and
additions to EnergyPlus models. It supports multiple workflow
pathways, including genEPJ list-based templates, direct epJSON
manipulation, and integrations with external tools such as eppy,
OpenStudio, and Modelkit.

The default input format is the genEPJ object list. When required,
helper functions translate data to and from epJSON or eppy-based
representations.
"""
import sys
...
from os import chdir, environ, getenv, getcwd, remove, system
from time import time
from os.path import isfile
from pathlib import Path
from subprocess import run

# IDF and epJSON helpers. The relative imports are used for normal
# package installation; the fallback keeps older script-style execution
# working when files are run directly from the repository.
try:
    from .pylib.templater import verbose, template_dict
    from .pylib.templater import templater as idf_templater
    from .pylib.epJSON_templater import (
        get_template_defaults_required,
        load_json_template,
        template_dict,
        templater,
    )
    ...
    from .resources import epjson_template_path
...
# Load the packaged epJSON template resource without relying on the
# current working directory. This addresses Windows installations where
# relative paths such as ``templates_9.0.1.json`` can fail.
epJSON_template = str(epjson_template_path())
...
def show_welcome(eplus_vers):
    """Display the startup banner for genEPJ."""
...
class Configuration:
    """Store EnergyPlus and workflow configuration settings."""
    def __init__(self):
        self.version = "25.1.0"
    ...
    def set_verbose_substitute(self, val):
...
class Building:
    """Store building-specific settings used during model generation."""
    def __init__(self):
...
def swap_EPPYIDF(data, args={}):
    ...
    * `new_data`: genEPJ/eppy data
    ...
    if 'IDD' in args :
        iddfile = args['IDD']
        if IDF is None:
            raise ModuleNotFoundError("The eppy package is required for swap_EPPYIDF.")
        IDF.setiddname(iddfile)
    else:
        printc("EPPY needs IDD file. Guessing from environment variables.", 'yellow')
        eplus_dir = getenv('ENERGYPLUS_DIR', '')
    ...
    if IDF is not None and type(data)==IDF:
...
def rm_skylights(objs_all, args={}):
    ...
    * `new_objs`: genEPJ object list with skylights removed (Python `List`)
    ...
    # Optionally keep only a fraction of skylights during testing.
...
def add_HVAC_VAVelec(objs_all, args={ 'use_DOAS': False } ):
    ...
    * `new_objs`: genEPJ object list with VAVelec HVACTemplate (Python `List`)
    ...
    # Occupancy-based outdoor-air scheduling may require project-specific rules.
...
#    NodeList integration for HPWH can be handled in future maintenance work.
...
def dispatch_JSON(objs_all, args={} ):
    ...
    def _take_first_key(mydict):
        return list(mydict.keys())[0]
    def _take_first_value(mydict):
...
def add_internalmass(objs_all, args={}):
    ...
    * `new_objs`: genEPJ object list with `InternalMass` objects added to specified `Zones`(Python `List`)
    ...
    objs_zon=filter_IDF_objs_raw(objs_all, 'Zone')
    # Similar logic may be extended to related functions if needed.
    # VD: Add specific zones if specified
    if 'zone_selective_add' in args:
        zone_selective_add = args['zone_selective_add']
...
# Legacy experimental block retained for reference.
...
def add_vent_loads(objs_all, args={ 'kitch':1, 'bath':2} ): # Number of kitches/bathrooms
    ...
    * `new_objs`: genEPJ object list with exhaust fan ventilation objects (Python `List`)
    ...
    for nm in all_zones:
        ...
        # `units=1.0` is retained here for backward compatibility.
        units=1.0
        # Calc approx number of units
...
def add_ExtShading(objs_all, args={ 'wid': 0.76} ):
    ...
    * `new_objs`: genEPJ object list with newly added Exterior Shading objects (Python `List`)
    ...
    # This block is kept in its current form for compatibility.
    temp_extshd="""
...
#
# NOTE: This path is currently unsuitable because -z-axis zones are already linked to zonelists.
...
def check_model(objs_all, args={"interactive": True, "epw": "CAN_ON_Ottawa.716280_CWEC.epw", "idf": None}):
    ...
    * `new_objs`: genEPJ object list (identical to input to allow for function chaining in genEPJ)
    ...
    wf_dir = getenv('ENERGYPLUS_WEATHER', '')
    ...
    if ep_dir:
        eplus_version_parts = Path(ep_dir).name.split('-')
        if len(eplus_version_parts) >= 3:
            vers_ma = eplus_version_parts[1]
            vers_mi = eplus_version_parts[2]
            _vers_obj = _get_first(try2filter('Version')).replace(';', '')
            printc(
                "... VERSION: Comparing: `%s' to `%s'"
                % (get_obj_name(_vers_obj), vers_ma + '.' + vers_mi),
                'yellow',
            )
            assert_compare(
                'VersionIDF_vs_VersionEnviro',
                get_obj_name(_vers_obj) == vers_ma + '.' + vers_mi,
            )
        else:
            printc("Skipping EnergyPlus version comparison; ENERGYPLUS_DIR format not recognized", 'yellow')
    else:
        printc("Skipping EnergyPlus version comparison; ENERGYPLUS_DIR is not set", 'yellow')
    ...
    wf_dir = getenv('ENERGYPLUS_WEATHER', '')
    _e = join(wf_dir, args['epw']) if wf_dir and args.get('epw') else ''
    _d = _e.replace('epw', 'ddy') if _e else ''
    assert_compare("EPW exists", bool(_e) and isfile(_e))
    assert_compare("DDY exists", bool(_d) and isfile(_d))
    if _e and isfile(_e):
        assert_compare("UTF8 in EPW", not has_utf8(_e))
    else:
        printc("Skipping UTF8 in EPW check; EPW file not found", 'yellow')
    if _d and isfile(_d):
        assert_compare("UTF8 in DDY", not has_utf8(_d))
    else:
        printc("Skipping UTF8 in DDY check; DDY file not found", 'yellow')
