#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Utility helpers for genEPJ.

This module collects general-purpose helper functions used across genEPJ
for file handling, EnergyPlus object parsing, JSON translation, workflow
execution, and small mathematical or text-processing utilities.
"""
...
# NaN was deprecated as of NumPy 1.20.
try:
...
import json
# Maintain the order of modified JSON to match the source JSON.
from collections import OrderedDict
from pathlib import Path

# Temporary files used to process EPPY.
import tempfile
...
def zeroIfNone(myval):
    "Return 0 when the supplied value is ``None``."
    if myval is None:
        return 0
    return myval
...
def join_lists(a, b):
    "Join two lists and return the merged list."
    c = []
    c.extend(a)
    c.extend(b)
    return c
...
def removeWhiteTrailSpaces(txtdat):
    "Remove trailing spaces from a provided text blob."
    r = re.compile(r"\s+$", re.MULTILINE)
    return r.sub("", txtdat)

def rotate2D(theta):
    "Return a rotation matrix for the angle ``theta``."
...
def cross3D(a, b):
    "Perform a cross-product multiplication on 3D coordinates."
    c = [a[1]*b[2] - a[2]*b[1],
         a[2]*b[0] - a[0]*b[2],
         a[0]*b[1] - a[1]*b[0]]
    return array(c)
...
def norm3D(a):
    "Return the normalized length of vector ``a``."
    return a / len3D(a)

# NOTE- mark for deletion
def adv_nl_split(mystr):
    "Split lines while handling Windows and Unix newlines."
    lines=mystr.split('\n')
    if len(lines)<1 :
...
def printc(s, color='default'):
    "Print text using terminal colour codes."
    print(textColors[color] + s + textColors['default'])

def mkcolor(mytxt, color='default'):
    "Return text with terminal colour codes applied."
...
wfile_dir = getenv('ENERGYPLUS_WEATHER', '')
...
def rm_unused_objs(myobjs):
    ...
    for i,myobj in enumerate(myobjs):
        ...
        obj_name = re.escape(obj_name.strip('\n'))
...
def write_file(objs_all, nm):
    ...
    elif ( (IDF is not None and type(objs_all)==IDF) and ('.json' in nm.lower())): # extension changed in deploy_if_function_list. Rename as IDF
        newfile=nm.replace(ext, '.idf')
    else:
        newfile=nm
    ...
    if '.idf' in newfile.lower():
        ...
        if IDF is not None and type(objs_all)==IDF :
...
def get_features(myfile):
    ...
    if getenv('GENEPJ_VERBOSE_IMPORT', '') == '1':
